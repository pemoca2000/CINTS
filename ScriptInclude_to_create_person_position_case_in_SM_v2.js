...
VASUtil.prototype = {
    ...
    // ============================================================
    // NEW: SM SOAP INTEGRATION METHODS
    // ============================================================

    /**
     * Main entry point called by the Applicant Business Rule.
     *
     * What it does:
     ...
     *  4) Writes handles back into Applicant + Case records
     *
    ...
    createPersonPositionCaseFromApplicant: function(applicantGR) {
        try {
            ...
            if (!caseGR) {
                gs.warn('[VASUtil] No related case found for applicant: ' + applicantGR.getUniqueValue());
            ...
            if (caseGR.sm_case_handle || caseGR.sm_position_handle) {
                gs.info('[VASUtil] Case already has SM handles. Skipping. case=' + caseGR.getUniqueValue());
            ...
            // 3) Apply HTTP Basic Auth Profile by name (as per updated info)
            ...
            if (!handles.personHandle || !handles.positionHandle || !handles.caseHandle) {
                gs.error('[VASUtil] SOAP succeeded but missing one or more handles. applicant='
                    + applicantGR.getUniqueValue() + ', case=' + caseGR.getUniqueValue());
                gs.error('[VASUtil] Response body: ' + body);
            ...
            gs.info('[VASUtil] SM create succeeded. applicant=' + applicantGR.getUniqueValue()
            ...
            gs.error('[VASUtil] Exception in createPersonPositionCaseFromApplicant: ' + msg);
     ...
     *
     * Change the field name 'applicant' below if your reference field differs.
     */
    _getCaseForApplicant: function(applicantGR) {
        var c = new GlideRecord('x_g_cfm_vas_case');

        // IMPORTANT: Assumes the case has a reference field literally named "applicant"
    ...
    _applyHttpBasicAuthProfile: function(soap) {
        ...
        if (!profileSysId) {
            gs.warn('[VASUtil] Basic auth profile not found: VAS SM Dev Basic Auth Creds');
            return;
        }
    ...
    _buildSmsStdCreatePayload: function(caseGR, applicantGR) {
        ...
        // Pick best email source from applicant record
        // (Adjust priority if your business rules differ)
        var personEmail =
    ...
    _logFailure: function(applicantGR, caseGR, status, body) {
        gs.error('[VASUtil] SM create failed. HTTP ' + status
            + ' applicant=' + applicantGR.getUniqueValue()
            + ' case=' + caseGR.getUniqueValue());
        gs.error('[VASUtil] Response: ' + body);
    },
